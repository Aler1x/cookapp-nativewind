...
describe('Text Component', () => {
  ...
  it('applies correct Comfortaa font for different weights', () => {
    ...
    fontWeights.forEach(({ className, expectedFont }) => {
      const { getByText } = render(
        <Text className={className}>{className} Text</Text>
      );
      const textElement = getByText(`${className} Text`);
      
      expect(textElement.props.style).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ fontFamily: expectedFont })
        ])
      );
    });
  });
  ...
  it('removes font weight classes from className', () => {
    const { getByText } = render(
      <Text className="font-bold text-red-500">Bold Red Text</Text>
    );
  ...
  it('handles multiple font weight classes correctly', () => {
    const { getByText } = render(
      <Text className="font-light font-bold text-blue-500">Multiple Font Classes</Text>
    );
    ...
    expect(textElement.props.style).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ fontFamily: 'Comfortaa_700Bold' })
      ])
    );
  });
